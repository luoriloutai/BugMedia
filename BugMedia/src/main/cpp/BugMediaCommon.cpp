...
// 将图形进行缩放并移到视图的中间
void scaleCenter(EGLint &viewWidth, EGLint &viewHeight, EGLint &width, EGLint &picHeight) {
    GLint newWidth;
    GLint newHeight;
    getShowPictureSize(newWidth, newHeight, viewWidth, viewHeight, width, picHeight);
...
void moveToCenter(EGLint &viewWidth, EGLint &viewHeight, GLint &picWidth, GLint &picHeight) {
    ...
    // 图像中心点，只是个相对于左下角的偏移。
    GLint centerX = (viewWidth - picWidth) / 2;
    GLint centerY = (viewHeight - picHeight) / 2;
    glViewport(centerX, centerY, picWidth, picHeight);
}